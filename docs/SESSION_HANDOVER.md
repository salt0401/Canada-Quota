...
| Item | State |
|---|---|
| **Watchdog run #6 failed** (2026-08-06) | Re-checked against the Actions API 2026-08-08, and the original entry here mislabelled it. The **run**-level conclusion is `failure`; the **job**-level conclusion is `cancelled`. Both sit on the same run, which is how it first got written up as "cancelled, not failed" — quote whichever you mean. The substance was right: the job never started. Its steps array is **empty** — not even `Set up job` — after sitting **15 minutes** (16:42:12Z → 16:57:14Z) on a workflow that normally finishes in ~17s. That is GitHub failing to allocate a runner. **The `concurrency` group is ruled out**: runs #5/#6/#7 are a day apart (Aug 5 16:27, Aug 6 16:42, Aug 7 15:46), so nothing existed to supersede it, and a concurrency kill is near-instant rather than 15 minutes. Harmless on the day — data was 3 days old against a 7-day limit, and **no issue has ever been opened on this repo** — and #7 succeeded the next day. **The real point stands: a run that never starts is a silent gap in the heartbeat.** Nothing checked that day and nothing said so. Worth a look if it recurs |
