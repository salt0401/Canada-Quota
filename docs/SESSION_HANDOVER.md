...
| **SSH from the previous laptop now times out** | Was working 2026-08-02, timing out 2026-08-08 from egress `31.14.249.144`. Per `meps-server-docs/docs/08`, a *timeout* means dropped upstream — the source IP is no longer permitted in the IONOS and/or Windows firewall rule. Only the IONOS account holder can fix the upstream half. **Irrelevant while you work on the box**, but it is why this handover could not end with a live SSH check |
